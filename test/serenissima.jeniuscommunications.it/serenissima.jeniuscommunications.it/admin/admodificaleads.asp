...
<%
	...
	if request("mod")="yes" and isnumeric(request("id"))=true then
		...
		text_provenienza=senza_apice(Request.Form("provenienza"))
    ...
    if request("tasto")<>"" then
        ...
        if session.SessionID<>"" then
            if errore="false" and trim(request("tasto"))="invia" then			
                    ...
                    sql="update contatti set cognome='"+text_cognome+"',nome='"+text_nome+"',mail='"+text_email+"',telefono='"+text_telefono+"',indirizzo='"+text_indirizzo+"',citta='"+text_citta+"',provincia='"+text_provincia+"',note='"+text_richiesta+"',provenienza='"+text_provenienza+"' where id=" & cdbl(request("id"))
		            ...
		            'conn.execute(query) 
					'log
					loggaVisSchede "modifica lead","modifica lead","lead","modifica lead di :" & text_cognome & "  " & text_nome,session("loginnominativo")
            ...
            <!-- Navigation -->
                ...
                <div class="row">
                    <div class="col-lg-12"> 
							...
							<!--#include file=_leadpreventivimodifica.inc-->
