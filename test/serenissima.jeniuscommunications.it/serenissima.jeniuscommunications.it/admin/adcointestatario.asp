...
<%  
	...
	session("titolonavigazione")="INSERIMENTO COINTESTATARIO"
	...
	if request("tasto")<>"" then	
			...
			if verrore=" " then 
				strsqlupdate="update pratiche_cointestatario set cognome='" & text_cognome & "',nome='" & text_nome & "',provincia='" & text_provincia & "',tipocliente='" & text_tipolavoratore & "',mail='" & text_email & "',codfiscale='" & text_codicefiscale & "',cap='" & text_cap & "',citta='" & text_citta & "',indirizzo='" & text_indirizzo & "',mobile='" & text_telefono & "',cittadinanza='" & text_cittadinanza & "',statocivile='" & text_statocivile & "',professione='" & text_professione & "',reddito=" & text_reddito & ",disponibilita=" & text_disponibilita & " where idpratica=" & session("idpratica")
				conn.Execute(lcase(strsqlupdate))
				strsqlppf="update pratiche_finanziarie  set cognome='" & text_cognome & "',nome='" & text_nome & "' where nometabella='pratiche_cointestatario' and idpratica=" & session("idpratica")
		        conn.Execute(lcase(strsqlppf))
				'response.write(strsqlupdate)
				'response.end
				loggaVisSchede "insert/modifica cointestatario","pratiche_cointestatario",session("idpratica"),"pratiche_cointestatario",session("loginnominativo")
                    ...
                    <div class="col-lg-12"> 
                            ...
                            <div class="panel-body">
                                <div class="panel-group" id="accordion">
									...
									<form action="adcointestatario.asp?tasto=invia" method="post" id="formuu" name="formuu">
									...
									<% 
															...
															Grid.SQL=""
															Grid.SQL = "select id,idpratica,idpratica as idcanc,nometabella,tipologia,debitoresiduo,importorata,scadenza,istituto,note from pratiche_finanziarie where nometabella='pratiche_cointestatario' and idpratica=" & session("idpratica") & " group by importorata order by importorata desc"
															Grid.Cols("idpratica").AttachExpression "<a class=navy11 HREF=""adinsertfinan.asp?nometabella=pratiche_cointestatario&tipooperazione=insert&idpratica={{idpratica}}&id={{id}}""><center><img src='images/edit.gif' border='0'></center></a>"
															Grid.Cols("idcanc").AttachExpression "<a class=navy11 HREF=""adinsertfinan.asp?nometabella=pratiche_cointestatario&tipooperazione=delete&idpratica={{idpratica}}&id={{id}}""><center><img src='images/elimina.png' border='0'></center></a>"
