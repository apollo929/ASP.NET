...
'verivica della scelta
cat=request("scelta")
if cat<>"tutto" then
	if provincia<>"" then
		Set interventi=Server.CreateObject("ADODB.Recordset")
		sqlin="Select * from preventivi where provincia like '%" & provincia & "%' and telefono not like '%mail%'  order by id desc "
		interventi.open sqlin,conn
		if interventi.EOF then 
		'response.Redirect("http://www.costruttori.it/interventi/interventi.asp")
		end if
	else
		Set interventi=Server.CreateObject("ADODB.Recordset")
		sqlin="Select * From preventivi where (categorie='" & cat & "' or dettagli like '%" & cat & "%' and telefono not like '%mail%' ) order by id desc "
		interventi.open sqlin,conn
	end if
else
		Set interventi=Server.CreateObject("ADODB.Recordset")
		sqlin="Select * From preventivi where telefono not like '%mail%' order by id desc limit 100 "
		interventi.open sqlin,conn
end if
                                    ...
                                    <div class="panel panel-default">
                                        <div class="panel-heading">
                                            <h4 class="panel-title">
											 <%if provincia="" then%>
                                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">Ultimi preventivi di TUTTA ITALIA</a>
											<%else%>
												<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">Ultimi preventivi di <%=ucase(provincia)%></a>
											<%end if%>		
                                            </h4>
                                        </div>
