...
session("titolonavigazione")="INSERIMENTO DOCUMENTI COINTESTATARIO (2)"
if trim(session("idpratica"))="" then response.redirect "adindex.asp"

'controllo form
sqlcall="select idpratica,data,nome,cognome,tipocliente,cittadinanza from pratiche_cointestatario2 where idpratica=" & trim(session("idpratica")) & " limit 1"
...
if text_tipocliente="" then response.redirect ("adcointestatario2.asp?idpratica=" & session("idpratica")) 
...
if lcase(trim(request("promemoria")))="si" then 
	...
	session("datacointestatario2")=text_dataapp
...
sqlinfo="select tipodocumento from pratiche_documenti where nometabella='pratiche_cointestatario2' and tipodocumento<>'generale' and idpratica=" & session("idpratica") & " limit 1"
...
if valore=0 then			
	...
	conn.execute("delete from pratiche_documenti where nometabella='pratiche_cointestatario2' and nomefile='' and idpratica='" & session("idpratica") & "'")
	Set tbtd=Server.CreateObject("ADODB.Recordset")
	tbtd.open sqltd,conn	
	do while not tbtd.eof
	conn.execute("insert into pratiche_documenti (idpratica,nomedocumento,tipodocumento,nometabella) values ('" & session("idpratica") & "','" & senza_apice(tbtd("nomedocumento")) & "','" & senza_apice(tbtd("tipodocumento")) & "','pratiche_cointestatario2')")
	tbtd.movenext
	loop
	tbtd.close
end if
            ...
            <!-- Navigation -->
                ...
                <div class="row">
                    <div class="col-lg-12"> 
							...
							<form action="addocumentiuploadcointestatario2.asp?promemoria=si" method="post" id="formpromo" name="formpromo" >
							...
							<select  type="select" id="note" name="note">
									<option selected value="PRATICA N <%=session("idpratica")%> (COINTESTATARIO2)">PRATICA N <%=session("idpratica")%> (COINTESTATARIO2)</option>
							</select>
							</td>
							</tr>
							<tr><td colspan="2" height="1px"><hr></td></tr>
							...
							<form action="uploadallfilecointestatario2.asp" method="post" enctype="multipart/form-data">							
							<%
							
						    sqltd="select * from pratiche_documenti where nometabella='pratiche_cointestatario2' and idpratica='" & session("idpratica") & "' group by nomedocumento order by id asc "
							'response.write(sqltd)
							'response.end
							Set tbtd=Server.CreateObject("ADODB.Recordset")
							tbtd.open sqltd,conn	
							...
							%>
								...
								%>
								<tr><td colspan="2" height="3px">Firma ______________________________ consegna il <%=session("datacointestatario2")%></td></tr>
								</table>
								</div>
