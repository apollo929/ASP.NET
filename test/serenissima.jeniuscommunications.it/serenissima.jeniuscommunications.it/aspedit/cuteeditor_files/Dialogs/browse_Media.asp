...
<%
	...
	If InStr(Lcase(current_Path),Lcase(trim(MediaGalleryPath))) <= 0 or MediaGalleryPath ="" then
    ...
    If DemoMode <> "true" Then
	    ...
	    Select Case action
		    Case "deletefile"  
                ...
                response.redirect("browse_Media.asp?setting=" & request.querystring("setting") & "&loc=" & request.querystring("loc") & "&MP=" & request.querystring("MP") & "&Theme=" & request.querystring("Theme"))
     	    Case "renamefile"  
...
Function Showbrowse_Img(spec)
	...
	For Each fol In sf 'add the html for the folders
		...
		str_openfolderEvent = "onclick=""parent.SetUpload_FolderPath('"&current_Path&folpath&fol.name&"');location.href='browse_media.asp?"&setting&"&loc=" & folpath & fol.name & "&Theme="&Theme&"&MP="&current_Path&"';"""
	...
	'	s = s & "<td NOWRAP style=""cursor:pointer;;VERTICAL-ALIGN: middle"" "&str_openfolderEvent&">" & FormatSize(fol.size) & "</td>" 
...
Function ValidMedia(str_FileName)
	...
	for i = 0 to ubound(Array_MediaFilters)
		if trim(Array_MediaFilters(i)) = "."&temp then
			ValidMedia = true
			exit for
		else
			ValidMedia = false
...
<head>
	...
	<script type="text/javascript">
		var folderpath = "browse_media.asp?<%=setting %>&MP=<%=current_Path %>&Theme=<%=Theme%>";
