...
<%
	...
	Function Showbrowse_Img(spec)
		...
		Set sf = f.SubFolders
		s = s & "<div style='HEIGHT: 120px;overflow:auto'>"
		...
		For Each fol In sf 'add the html for the folders
			...
			s = s & "<tr><td class=""cursor"" onclick=""SetUpload_imagePath('"&p&"');location.href='browse_Img_gallery.asp?"&setting&"&loc=" & folpath & fol.name & "&Theme="&Theme&"&GP="&c_ImageGalleryPath&"';"">" & vbcrlf
			s = s & "<img src=""../Images/closedfolder.gif"" style=""vertical-align:middle;margin-right:4px;"">" & fol.name & "&nbsp;&nbsp;</td>"
			s = s & "</tr>" & vbcrlf	
		Next
		...
		' Show our table
		s = s & "<div style='height:300px;margin-top:15px'><table width='100%' valign='top'>"
...
<body>
	...
	<iframe src="upload.asp?<%=setting %>&Theme=<%=Theme%>&FP=<%=c_ImageGalleryPath & folpath%>&Type=Image" id="upload_image" frameborder="0" scrolling="auto"></iframe>
    <%
		end if
	%>	
	</div>
