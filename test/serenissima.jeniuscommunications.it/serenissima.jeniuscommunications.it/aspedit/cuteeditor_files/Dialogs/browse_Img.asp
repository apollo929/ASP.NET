...
<%
	...
	If InStr(Lcase(current_Path),Lcase(trim(ImageGalleryPath))) <= 0 or ImageGalleryPath ="" then
    ...
    If DemoMode <> "true" Then
	    ...
	    Select Case action
		    Case "deletefile"  
                ...
                response.redirect("browse_Img.asp?setting=" & request.querystring("setting") & "&loc=" & request.querystring("loc") & "&MP=" & request.querystring("MP") & "&Theme=" & request.querystring("Theme"))
		    Case "renamefile"  
...
Function Showbrowse_Img(spec)
	...
	For Each fol In sf 'add the html for the folders
		...
		str_openfolderEvent = "onclick=""parent.SetUpload_FolderPath('"&current_Path&folpath&fol.name&"');location.href='browse_Img.asp?"&setting&"&loc=" & folpath & fol.name & "&MP="&current_Path&"&Theme="&Theme&"';"""
		s = s & "<tr onMouseOver=""row_over(this)"" onMouseOut=""row_out(this)"" class=""cursor"">"
		s = s & "<td "&str_openfolderEvent&"><img src=""../Images/closedfolder.gif"" style=""""></td>" & vbcrlf
		s = s & "<td "&str_openfolderEvent&">" & vbcrlf
		s = s & fol.name & "&nbsp;</td>" & vbcrlf
		's = s & "<td "&str_openfolderEvent&">" & FormatSize(fol.size) & "</td>" 
	...
	For Each fil In fc 'add the html for the files
		If (InStr(fil.name, "'" ) = 0) Then
			...
			If ValidImage(filename) Then
				...
				s = s & "<td ><img src=""../Images/" & GetExtension(fil.Name) & ".gif"" border=""0"" alt="""" style="""" /></td>" & vbcrlf
...
<head>
	...
	<script type="text/javascript">
		var folderpath = "browse_Img.asp?<%=setting %>&MP=<%=current_Path %>&Theme=<%=Theme%>";
