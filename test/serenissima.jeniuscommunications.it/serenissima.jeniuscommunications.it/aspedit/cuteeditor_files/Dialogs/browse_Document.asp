    ...
    If DemoMode <> "true" Then
        ...
        Select Case action
	        Case "deletefile"  
                ...
                response.redirect("browse_Document.asp?setting=" & request.querystring("setting") & "&loc=" & request.querystring("loc") & "&MP=" & request.querystring("MP") & "&Theme=" & request.querystring("Theme"))
	        Case "renamefile"  
	        ...
	        Case "createfolder"  
		        ...
		        If fso.FolderExists(folderPath) = false and fso.FileExists(folderPath) = false Then 
			        ...
			        Else
				        exMessage = "Folder """ & folderPath & """ is created"
	                    Response.Write "<script language=""javascript"">parent.Refresh('"&folpath&"');</script>"
			        End If
		        Else
...
Function Showbrowse_Img(spec)
	...
	Set sf = f.SubFolders
	s = s & "<table border=""0"" cellspacing=""1"" cellpadding=""1"" id=""FoldersAndFiles"" style=""width:100%; vertical-align:middle"" class=sortable>"
	s = s & "<tr onMouseOver=""row_over(this)"" onMouseOut=""row_out(this)"" bgcolor=""#f0f0f0"">"
	s = s & "<th width=22 class=""cursor""><img src=""../Images/refresh.gif"" title=""refresh"" onclick=""parent.Refresh('"&folpath&"');"" onMouseOver=""parent.CuteEditor_ColorPicker_ButtonOver(this);"" class=dialogButton></th>"
	s = s & "<th width=140 Class=""filelistHeadCol""><b>Name</b></th>"
	s = s & "<th width=60 Class=""filelistHeadCol""><b>Size</b></th>"
	s = s & "<th width=60 Class=""filelistHeadCol""><b>Modified</b></th>"
	s = s & "<th width=60 Class=""filelistHeadCol""><b>Created</b></th>"
	s = s & "<th width=15 Class=""filelistHeadCol""></th>"
	s = s & "<th width=80 Class=""filelistHeadCol""><b>Type</b></th>"
	...
	End If
	s = s & "</tr>"
	s = s & "<tr class=""cursor"" onMouseOver=""row_over(this)"" onMouseOut=""row_out(this)"" onclick=""Editor_upfolder();"">"
	s = s & "<td><img src=""../Images/parentfolder.gif"" title=""Go up one level""></td>"
	s = s & "<td>...</td>"
	s = s & "<td></td>"
	s = s & "<td></td>"
	s = s & "<td></td>"	
	...
	For Each fol In sf 'add the html for the folders
	  ...
	  ' f_Tooltip=f_Tooltip & "<nobr>Compressed: "&CBool(f_Attributes And 2048)&"</nobr><br>"
		...
		s = s & "<td "&str_openfolderEvent&"><img src=""../Images/closedfolder.gif""></td>" & vbcrlf
		...
		s = s & "<td "&str_openfolderEvent&" style=""text-align:center"">" & f_DisplayAttribute & "</td>" 
		s = s & "<td>Folder</td>" 
		if CBool(AllowDelete) Then
			s = s & "<td style=""text-align:center""><img src=""../Images/delete.gif"" onclick=""deletefolder('" & current_Path & folpath  & fol.name & "')"" title=""Delete""></td>" 
		End If
		if CBool(AllowRename) Then	
			s = s & "<td style=""text-align:center""><img src=""../Images/edit.gif"" title=""Rename"" onclick=""renamefolder('" & current_Path & folpath  & fol.name & "')""></td>" 
		End If		
		if CBool(AllowDelete) Then
			s = s & "<td></td>" 
		End If
		...
		If (InStr(fil.name, "'" ) = 0) Then
			...
			If ValidMedia(filename) Then	
			  ...
			  ' f_Tooltip=f_Tooltip & "<nobr>Compressed: "&CBool(f_Attributes And 2048)&"</nobr><br>"
				...
				s = s & "<tr class=""cursor"" onMouseOver=""row_over(this);"" onMouseOut=""row_out(this);"" onclick=""parent.row_click('" & current_Path & folpath & fil.name & "','" & f_Tooltip & "'); "">"
				s = s & "<td><img src=""../Images/" & GetExtension(fil.Name) & ".gif"" border=""0"" alt=""""></td>" & vbcrlf
				s = s & "<td class=""namecell"">" & f_Name & "</td>" & vbcrlf
				s = s & "<td>" & FormatSize(f_Size) & "</td>" 
				s = s & "<td>" & FormatDateTime(f_Modified,2) & "</td>" 
				s = s & "<td>" & FormatDateTime(f_Created,2) & "</td>" 
				s = s & "<td  style=""text-align:center"">" & f_DisplayAttribute & "</td>" 
				s = s & "<td>" & f_Type & "</td>" 
				if CBool(AllowDelete) Then
					s = s & "<td style=""text-align:center""><img src=""../Images/delete.gif"" title=""Delete"" onclick=""deletefile('" & current_Path & folpath  & fil.name & "')""></td>" 
				End If
				if CBool(AllowRename) Then	
					s = s & "<td style=""text-align:center""><img src=""../Images/edit.gif"" title=""Rename"" onclick=""renamefile('" & current_Path & folpath  & fil.name & "','"&fil.name&"')""></td>" 
				End If
				if CBool(AllowDelete) Then
				    s = s & "<td style=""text-align:center""><img src=""../Images/download.gif"" title=""Download"" onclick=""downloadfile('" & current_Path & folpath  & fil.name & "')""></td>" 
				End If
				s = s & "</tr>" & vbcrlf
			End If
		End If
	...
	<script type="text/javascript">
		...
		function deletefile(path)
		{
			if (confirm("Delete File " + path + "?")) {
				...
				setTimeout(function(){parent.ResetImage();}, 100);
			}	
		}
