...
<%
	...
	If InStr(Lcase(current_Path),Lcase(trim(TemplateGalleryPath))) <= 0 or TemplateGalleryPath ="" then
	...
	Set fso = Server.CreateObject("Scripting.FileSystemObject")
	    Select Case action
		    Case "deletefile"  
                ...
                response.redirect("browse_Template.asp?setting=" & request.querystring("setting") & "&loc=" & request.querystring("loc") & "&MP=" & request.querystring("MP") & "&Theme=" & request.querystring("Theme"))
	  	    Case "renamefile"  
	...
	If(exMessage<>"") Then 
		'Response.Write "<script language=""javascript"">parent.alert('" & exMessage & "');</script>"
	End If
...
Function Showbrowse_Img(spec)
	...
	s = s & "<th width=21 class=""cursor""><img src=""../Images/refresh.gif"" title=""refresh"" onclick=""parent.Refresh('"&folpath&"');"" onMouseOver=""parent.CuteEditor_ColorPicker_ButtonOver(this);"" class=dialogButton></th>"
	...
	For Each fol In sf 'add the html for the folders
		...
		str_openfolderEvent = "onclick=""parent.SetUpload_FolderPath('"&current_Path&folpath&fol.name&"');location.href='browse_Template.asp?"&setting&"&loc=" & folpath & fol.name & "&Theme="&Theme&"&MP="&current_Path&"';"""
		...
		s = s & "<td "&str_openfolderEvent&">" & vbcrlf
		...
		if CBool(AllowDelete) Then
			s = s & "<td><img src=""../Images/delete.gif"" onclick=""deletefolder('" & current_Path & folpath  & fol.name & "')"" title=""Delete""></td>" 
		End If
		if CBool(AllowRename) Then	
			s = s & "<td><img src=""../Images/edit.gif"" title=""Rename"" onclick=""renamefolder('" & current_Path & folpath  & fol.name & "')""></td>" 
		End If		
		s = s & "</tr>" & vbcrlf
	Next
	Set fc = f.Files
	...
	For Each fil In fc 'add the html for the files
		If (InStr(fil.name, "'" ) = 0) Then
			...
			If ValidImage(filename) Then
				dim html
			'	html=LoadHTML(current_Path & folpath & fil.name)
			'	html=server.URLEncode(html)	
			'	html=server.HTMLEncode(html)							
				response.Write "<script language=""javascript"">parent.TargetUrl.value='"&html&"'</script>"				
				s = s & "<tr class=""cursor"" onMouseOver=""row_over(this)"" onMouseOut=""row_out(this)"" onclick=""parent.row_click('" & current_Path & folpath & fil.name & "'); "">"
				s = s & "<td><img src=""../Images/" & GetExtension(fil.Name) & ".gif"" border=""0"" alt=""""></td>" & vbcrlf
				s = s & "<td class=""namecell"" >" & vbcrlf
				s = s & fil.name & "</td>" & vbcrlf
				s = s & "<td>" & FormatSize(fil.size) & "</td>" 
				if CBool(AllowDelete) Then
					s = s & "<td><img src=""../Images/delete.gif"" title=""Delete"" onclick=""deletefile('" & current_Path & folpath  & fil.name & "')""></td>" 
				'	s = s & "<td><img src=""../Images/download.gif"" title=""Download"" onclick=""downloadfile('" & current_Path & folpath  & fil.name & "')""></td>" 
				End If
				if CBool(AllowRename) Then	
					s = s & "<td><img src=""../Images/edit.gif"" title=""Rename"" onclick=""renamefile('" & current_Path & folpath  & fil.name & "','"&fil.name&"')""></td>" 
				End If
				s = s & "</tr>" & vbcrlf
			End If
		End If
...
function FormatSize(fileSize) ' need to check later ........adam
	...
	function LoadHTML(ByVal FilePath)
		dim fso
		dim file
		dim fileContents
		...
		if fso.FileExists(FilePath) then
			...
			LoadHTML = fileContents
		else
			LoadHTML = "File " & FilePath & " doesn't exist"
		end if
	end function
...
<head>
	...
	<script type="text/javascript">
		var folderpath = "browse_Template.asp?<%=setting %>&Theme=<%=Theme%>&MP=<%=current_Path %>";
		...
		function Editor_upfolder(path) {
			...
			str="browse_Template.asp?<%=setting %>&Theme=<%=Theme%>&MP=<%=current_Path %>&loc=" + str + "&u=y";
			location.href = str;
