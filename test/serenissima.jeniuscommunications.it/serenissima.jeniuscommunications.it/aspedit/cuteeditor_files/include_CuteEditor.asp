...
Class CuteEditor
	...
	Sub GetBrowserType ()	
		...
		if InStr(1, userAgent, "MSIE", 1) > 0 AND InStr(1, userAgent, "mac", 1) = 0 AND InStr(1, userAgent, "Opera", 1) = 0 then
			if CInt(Trim(Mid(userAgent, inStr(1, userAgent, "MSIE", 1)+5, 3))) > 5 then
				s_BrowserType = "winIE"
			end if
		ElseIf inStr(1, userAgent, "Trident/", 1) then
			s_BrowserType = "Gecko"
	...
	Public Function GetString()
		...
		Else	
			strTemp = strTemp & " <!-- CuteEditor Version 6.7 "&ID&" Begin --> " & vbCRLF
			if Lcase(BrowserType)="safari12" then
			...
			strTemp = strTemp & "<table "&s&" id="""&ClientID&""" _IsCuteEditor=""True"" cellspacing=""0"" cellpadding=""0"" style=""background-color:"&s_BackColor&";border-color:"&s_BorderColor&";border-width:"&s_BorderWidth&"px;border-style:"&s_BorderStyle&";table-layout:auto;height:"&s_height&"px;width:"&s_width&"px;"">"
			...
			strTemp = strTemp & "<iframe id="""&ClientID&"_Frame"" src="""&GetURL("template.asp")&""" FrameBorder=""0"" class=""CuteEditorFrame"" spellcheck=""false"" style=""background-color:White;border-color:#dddddd;border-width:1px;border-style:Solid;height:100%;width:100%;""></iframe>"
			strTemp = strTemp & "</td></tr>"
	...
	Public function AddToolbarDropDownfromConfig(name,command,width)
		...
		if lcase(RenderRichDropdown) <> "true" then
    ...
    Public Function CleanUpHTMLCode(HTMLstring)
