...
var dlgurl='<%=FilePath%>/Dialogs/ColorPicker.Asp?<%=Request.ServerVariables("QUERY_STRING") %>&setting=<%= Request.Cookies("CESecurity") %>';
...
function element._cphtc_dlg()
{
	...
	{
		//FEATURE:change the color in ColorPicker.asp would fire the event so the changing would affect immediately , and it could be auto rollback
...
function CalcPosition(floate,e)
{
	...
	var spos=GetScrollPosition(GetStandParent(floate));
