...
baseurl=Session("sitourl") & "dettaglibuffettishop.asp?cod="

strsql = "SELECT * FROM prodotti where codice_buf<>'' order by rand() limit 2500 "
utcOffset=1
...
Do while not rs.eof
	if URLS<MAXURLS_PER_SITEMAP then
		...
		id_page=Server.URLEncode(rs("codice_buf"))
