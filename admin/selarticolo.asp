	...
	If not tbshop.EOF then
		...
		Do while not (tbshop.eof or (Counter > tbshop.PageSize))
		 	If len(tbshop("cimageurl"))>2 then
				...
				Response.Write "<img width=100 height=100 border=0 src='articoli/" & newimmagine & "'>"
			else
				Response.Write "<td   width=100 height=100 allign=middle vallign=middle bgcolor=" & color2 & ">"
				Response.Write "<img width=100 height=100 border=0 src=spacer.gif>"
			end if
			Response.Write "</td>"
			...
			Response.Write "<tr><td   height=50 valign=top>"
			 
			Response.Write "<b>" & tbshop("cname") & "</b><br>"
			Response.Write "</font>"
			Response.Write "</tr></td>"
			Response.Write "<tr><td   height=30 valign=baseline>"
			 
			Response.Write "<a      href=selarticolo.asp?art="& tbshop("catalogID") & "&act=ins>"
			Response.Write "<img src='img/seleziona.gif' border=0 >"
			Response.Write "</a>"
			Response.Write "</font>"
		...
		If pagina<>1 then
			...
			Response.Write "<img src=img/spacer.gif width=10 border=0 align=absmiddle>&nbsp;"
		end if
		for f=1 to npagine
			if pagina=f Then
				Response.Write "<b>(" & f & ")</b>&nbsp;"
			else
				Response.Write "<a  href=selarticolo.asp?pag=" & trim(f) & "&id=" & request("ID") & "&sid=" & request("SID") & ">"
				Response.Write "" & f & "&nbsp;"
				Response.Write "</a>"
			end if
		next
...
<html>
	...
	<body bgcolor="white" link="black" vlink="black">
		<form name="articoli">
			<table width="100%">
				<tr>
					<td   align=middle colspan=2><font face="Verdana" Size="2"><b>Selezionare l'Articolo</b></font></td>
				</tr>
				<tr>
