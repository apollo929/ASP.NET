...
<iframe src="browse_Img.asp?<%=setting %>&Theme=<%=Theme%>&GP=<%=Current_ImageGalleryPath%>" id="browse_Frame" frameborder="0" scrolling="auto" style="border:1.5pt inset;width:270px;height:246px"></iframe>		
					</td>
...
<input class="formbutton" type="button" value="   <%= GetString("Cancel") %>  " onclick="do_Close()" id="Button2" />
	    ...
	    function UploadSaved(sFileName,path){
		    ...
		    browse_Frame.location="browse_Img.asp?<%=setting %>&Theme=<%=Theme%>&GP="+path+"";	
		    row_click(sFileName);
	    }
    	
	    function Refresh(path)
	    {
		    browse_Frame.location="browse_Img.asp?<%=setting %>&Theme=<%=Theme%>&GP=<%=ImageGalleryPath%>&loc="+path+"";
	    }
