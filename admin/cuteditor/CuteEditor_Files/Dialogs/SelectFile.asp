...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	    ...
	    <%
		    ...
		    if Request.QueryString("FP") <> "" then
			    Current_FilesGalleryPath = Request.QueryString("DP")
		    End if
	    %>
	</head>
	<body>
...
<iframe src="browse_Document.asp?<%=setting %>&Theme=<%=Theme%>&DP=<%=Current_FilesGalleryPath%>" id="browse_Frame" frameborder="0" scrolling="auto" style="border: buttonshadow 1px solid; width:540px;height:280px"></iframe>		
					</td>
					<td>&nbsp;
					</td>
...
<input class="formbutton" type="button" value="   <%= GetString("Cancel") %>  " onclick="do_Close()" id="Button2" />
			...
			<script type="text/javascript">	
	            ...
	            function UploadSaved(sFileName,path){
		            ...
		            browse_Frame.location="browse_Document.asp?<%=setting %>&Theme=<%=Theme%>&DP="+path+"";	
		            row_click(sFileName,"");
	            }
            	
	            function Refresh(path)
	            {
		            browse_Frame.location="browse_Document.asp?<%=setting %>&Theme=<%=Theme%>&DP=<%=FilesGalleryPath%>&loc="+path+"";
	            }
