...
if request("del")<>"" then
	...
	response.Redirect "adutentiinternet.asp"
end if
...
if request("act")="" then	
	...
	if rs.eof then
		...
		if isnull(check_spese)=false then 
    ...
    <body>
        <div id="wrapper">
            ...
            <!-- Navigation -->
			  ...
			  <form action="adutentiinternetdettaglioint.asp?act=1&id=<%=request("id")%>&mod=<%=request("mod")%>" method="post" id=formuu name=formuu>
					...
					<td ><INPUT TYPE="TEXT" NAME="frequenza" VALUE="<%=text_frequenza%>" ID=TEXT9> </td>
					</tr>
					<tr>
					...
					<td ALIGN=LEFT><input type="image" src="images/salva1.png" id="image1" width=30px name="image1"></form>
					<form action="adutentiinternetdettaglioint.asp?id=<%=request("id")%>&del=<%=request("id")%>" method="post" id=formd name=formd>
					<input type="image" src="images/elimina.png" id="image1" width=30px name="image1">
					</form>
					</td></tr>	
					<tr><td  width=250><B >USCITA SENZA MODIFICHE</b><img width=200 height=1 src='images/frecciaw.gif'></td>
					<td ><a href=adutentiinternet.asp? ><img src='images/senzamodifiche.gif' border=0 ></a></td>
					</tr>
				  	</table>	  
