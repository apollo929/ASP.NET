...
'#### delete
if request("del")<>"" then
 	Sqldel="Delete From " & tabella & " where id=" & cint(request("id")) 
...
'#### PAGINA ####
if request("act")="" then	
	Sqlpro="Select * From " & tabella & " where id=" & cint(request("id")) 
	...
	if text_password<>"" then
		rs.open "select password2 from registrazione  where id<>" & cint(request("id")) & " and password2='" & text_password & "' union select password2 from registrazioneint where id<>" & cint(request("id")) & " and password2='" & text_password & "'",connm
		if not rs.eof then
			errore=true
	...
	if text_utente<>"" then
		rs.open "select utente1 from registrazione  where id<>" & cint(request("id")) & " and  utente1='" & text_utente & "' union select utente1 from registrazioneint where id<>" & cint(request("id")) & " and  utente1='" & text_utente & "'",connm
		if not rs.eof then
			errore=true
			rossi=rossi & "utente,"
...
<td valign=top width=550 height=100% bgcolor='moccasin'>
            ...
            <form action="dettaglioupdate.asp?act=1&id=<%=request("id")%>&mod=<%=request("mod")%>" method="post" id=formuu name=formuu>
					...
					<tr>
					<td ><b>Prezzo Spicers</b><font color="#FF0000">*</font></td>
					...
					<tr>
					<td ><b>Prezzo Isp</b><font color="#FF0000">*</font></td>
					...
					<tr>
					<td ><b>Prezzo Cwr</b><font color="#FF0000">*</font></td>
					...
					<form action="dettaglioupdate.asp?id=<%=request("id")%>&del=<%=request("id")%>" method="post" id=formd name=formd>
