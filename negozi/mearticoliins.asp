...
if request("cmd")="del" then
		...
		connm.Execute StrSQL	
		response.redirect "merchant_articoli.asp?merchant=" & merchant & "&act=new"
end if
...
'Se Errore redirect
If Len(errore)=2 Then
	WriteSession
	Response.redirect "mearticoli.asp?cmd=new&errore=errore&merchant=" & merchant
End If
...
if request("cmd")="new" then
		
				'Aggiunge il record anche sull'archivio articoli
				StrSQL="insert into shop_products (ccode,cname,cdescription,cprice,cpriceofferta,cimageurl,cimageurl1,cimageurl2,cimageurl3,cimageurl4,filepdf,coffmag,cstock,iva,taglia,colore,ccategory,cscategory,cliente,owner)"	
